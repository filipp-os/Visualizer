...
/**
 * The heading the robot is effectively facing at the startPoint itself —
 * used as the "previous end heading" for the first path in the sequence.
 */
export function getStartPointEffectiveHeading(
  startPoint: Point,
  firstLine?: Line,
): number {
  ...
  if (startPoint.heading === "tangential") {
    if (!firstLine || !firstLine.endPoint) return 0;
    const nextP =
      firstLine.controlPoints.length > 0
        ? firstLine.controlPoints[0]
        : firstLine.endPoint;
    const angle = getTangentAngle(startPoint, nextP);
    return startPoint.reverse
      ? transformAngle(angle + 180)
      : transformAngle(angle);
  }
  return 0;
...
/**
 * The heading the robot ends up facing right before path `lines[idx]`
 * begins — i.e. the end heading of whatever comes right before it
 * (the previous line, or the startPoint itself if idx is the first line).
 * Used to auto-populate a linear path's startDeg.
 */
export function computePreviousEndHeading(
  lines: Line[],
  idx: number,
  startPoint: Point,
): number {
  if (idx <= 0) return getStartPointEffectiveHeading(startPoint, lines[0]);
  const prevLine = lines[idx - 1];
  const prevPrevPoint: Point =
    idx - 1 === 0 ? startPoint : (lines[idx - 2].endPoint as Point);
  return getLineEndHeading(prevLine, prevPrevPoint);
}
